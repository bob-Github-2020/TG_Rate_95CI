#! /usr/bin/python3
# Last updated: 01-27-2023, by Guoquan (Bob) Wang

# The Python function is designed for calculating the rate of monthly MSL (b) and its 95%CI
# The detailed methodology is adressed in:
# Wang, G. (2023). The 95% confidence interval for the rates of relative sea-level derived from tide gauge data, submitted (2/3/2023)
...
## You need to install The Latest SSA, carefully read the following website. 
# https://pyts.readthedocs.io/en/latest/install.html
  ...
  #cd pyts
  #pip install .
## It may take a while install the Latest SSA on your Windows system
...
def cal_95CI(year,ts,TG,output,pltshow):
    ...
    Reference
    ---------
      Wang, G. (2023). The 95% confidence interval for the rates of relative sea-level derived from tide gauge data, submitted xxx
...
# -----------------------------------------------------------------------------
    ...
    #  It is necessary to add name to year,  
    #  otherwise "Rresult.params" will not identify [0], [1]
...
# -----------------------------------------------------------------------------
# Step 2: Setup the SSA model (Si), calculate b_P
...
# -----------------------------------------------------------------------------  
    ...
    # The SSA model for decomposition, 11-22-2022
    def ssa_model(y):
...
# -----------------------------------------------------------------------------
   ...
   # determine Tau, autocorrelation time , Eq. 11       
    ...
    Nfit=len(ri)
    tao = 1 + 2*sum            # Eq.11
    Neff = int(Nfit/tao)          # Eq.10
    SEbc=np.sqrt(tao)*SEb      # Eq.12, same as SEbc=np.sqrt(N/Neff)*SEb
    print('Nfit=', Nfit, 'tao=', tao, 'Neff=', Neff, 'SEb=', SEb, 'SEbc=', SEbc)
...
# -----------------------------------------------------------------------------
    if output == 'on':
       ...
       # This is just a draft figure for illustrating purposes
       ...
       ax1.plot(ts, 'o-', label='TS')
       ax1.plot(Li, '-', label='Trend')
       #ax1.plot(y_con, 'o-', label='De_Linear')
       ax1.plot(Pi, '-', label='Periodics')
       ...
       plt.suptitle('Singular Spectrum Analysis'+TG, fontsize=20)
       ...
       fig.suptitle('Decomposition of TG-Derived Monthly MSL Time Series: PSMSL ID '+ TG, size=16,  y=0.93);
       ...
       fig1.set_title('(a) Monthly MSL timr series y(i) & Linear trend L(i)')
